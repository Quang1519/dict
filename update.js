...
class enen_Cambridge {
    ...
    async findYoudao(word) {
        ...
        let base = 'https://translate.google.com/?hl=vi&sl=en&tl=vi&text=';
        let url = base + encodeURIComponent(word) + '&op=translate';
        let doc = '';
        try {
            let data = await api.fetch(url);
            let parser = new DOMParser();
        ...
        function getYoudao(doc) {
            ...
            let defNodes = doc.querySelectorAll('.eyKpYb');
            //if (!defNodes || !defNodes.length) 
            return notes;
